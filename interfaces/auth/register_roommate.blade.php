...
@section('title', 'Register as Roommate')
...
@section('content')
    ...
    <div class="hero page-inner overlay d-flex align-items-center" style="background-image: url('{{ asset('assets/images/hero_bg_1.jpg') }}'); min-height: 300px;">
        <div class="container text-center text-white">
            <h1 class="display-4 text-white fw-bold">Register as Roommate </h1>
        </div>
    </div>
    ...
    <div class="registration-container section">
        <div class="registration-box">
            ...
            <form action="{{ route('website.register_submit') }}" method="POST">
                ...
                <!-- Hidden Account Type -->
                <input type="hidden" name="acc_type" value="roommate">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="acc_age" class="form-control" value="{{ old('acc_age') }}">
                    @error('acc_age')
                    <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="acc_smoking" class="form-control">
                        <option value="" selected>-- Select --</option>
                        <option value="1" {{ old('acc_smoking') === '1' ? 'selected' : '' }}>Yes</option>
                        <option value="0" {{ old('acc_smoking') === '0' ? 'selected' : '' }}>No</option>
                    </select>
                    @error('acc_smoking')
                    <div class="text-danger">{{ $message }}</div>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="acc_status" class="form-control" value="{{ old('acc_status') }}">
                    @error('acc_status')
                    <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
